...
from spatial_utils.data_process import butterworth_filter
...
def get_trial_de_feature_spatial(new_trails):
    
    features = []
    
    for trial in new_trails:
        feature = compute_DE(trial) # 对于每个trail计算差分熵特征
        features.append(feature)
    
    features = np.asarray(features)
    features = features.swapaxes(0, 2)  # 将数组0维和2维交换位置

    new_features = []
    for chan_idx in range(31):
        bands = []
        for band_idx in range(5):
            band_data = features[chan_idx, band_idx]
            if len(band_data) <= 30:
                band_data = (band_data - np.mean(band_data)) / np.std(band_data) + 0.5  # 标准化
                band_data = (band_data - min(band_data)) / (max(band_data) - min(band_data))    # 归一化
                band_data = np.pad(band_data, (0, 30 - len(band_data)), 'constant',     # 进行0填充，使其长度为30
                                                constant_values=(0, 0))
            bands.append(band_data)
        new_features.append(bands)
